...
from constants import FM_HEATMAP_CLASS_COUNT, FM_CAM_TYPE, G_CAM_TYPE


def get_model_pred(model, art_img_tensor, hm_type):
    grad_list = []
    act_list = []
    ...
    hook_backward = model.attention_module.register_full_backward_hook(hook_backward, prepend=False)
    hook_forward = model.attention_module.register_forward_hook(hook_forward, prepend=False)
